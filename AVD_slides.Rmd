...
non_hols <- birthsbyday 
non_hols$rel_count[c( xmas, boxday, nyd)] <- NA
ggplot2::ggplot(data = non_hols) +
  ggplot2::geom_line(mapping = ggplot2::aes(x = doy,y = rel_count)) + 
  ggplot2::ylab("mean birth count") + 
  ggplot2::xlab("day of year")
```
...
## Bootstrap test
$$ H_0: \text{all days have same expected count, up to occurance.}$$
$$vs.$$
$$H_1: \text{At least one day has a different count, up to occurance.}$$
- under $H_0$ the observed counts could have happened on any day
- If H_0 is correct then we can pick any 22 of the daily counts for any of the days.
